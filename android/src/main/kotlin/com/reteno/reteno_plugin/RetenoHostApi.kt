...
enum class NativeDeviceTokenHandlingMode(val raw: Int) {
  AUTOMATIC(0),
  MANUAL(1);

  companion object {
    fun ofRaw(raw: Int): NativeDeviceTokenHandlingMode? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
data class NativeInAppCustomData (
  val url: String? = null,
  val source: String,
  val inAppId: String,
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NativeInAppCustomData {
      val url = pigeonVar_list[0] as String?
      val source = pigeonVar_list[1] as String
      val inAppId = pigeonVar_list[2] as String
      val data = pigeonVar_list[3] as Map<String?, String?>
      return NativeInAppCustomData(url, source, inAppId, data)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is NativeInAppCustomData) {
      return false
    }
    if (this === other) {
      return true
...
/** Generated class from Pigeon that represents data sent in messages. */
data class NativeDefaultNotificationChannelConfig (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NativeDefaultNotificationChannelConfig {
      val name = pigeonVar_list[0] as String?
      val description = pigeonVar_list[1] as String?
      val showBadge = pigeonVar_list[2] as Boolean?
      val lightsEnabled = pigeonVar_list[3] as Boolean?
      val vibrationEnabled = pigeonVar_list[4] as Boolean?
      return NativeDefaultNotificationChannelConfig(name, description, showBadge, lightsEnabled, vibrationEnabled)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
private open class RetenoHostApiPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      130.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          NativeDeviceTokenHandlingMode.ofRaw(it.toInt())
        }
      }
      131.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
      ...
      132.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          NativeInAppCustomData.fromList(it)
        }
      }
      133.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
      ...
      141.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          NativeDefaultNotificationChannelConfig.fromList(it)
        }
      }
      142.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?)   {
    when (value) {
      ...
      is NativeDeviceTokenHandlingMode -> {
        stream.write(130)
        writeValue(stream, value.raw.toLong())
      }
      ...
      is NativeDefaultNotificationChannelConfig -> {
        stream.write(141)
        writeValue(stream, value.toList())
      }
...
interface RetenoHostApi {
  fun initWith(accessKey: String, lifecycleTrackingOptions: NativeLifecycleTrackingOptions?, isPausedInAppMessages: Boolean, useCustomDeviceIdProvider: Boolean, customDeviceIdValue: String?, isDebug: Boolean, deviceTokenHandlingMode: NativeDeviceTokenHandlingMode, defaultNotificationChannelConfig: NativeDefaultNotificationChannelConfig?)
  ...
  fun requestPushPermission(provisional: Boolean, callback: (Result<Boolean>) -> Unit)
  fun updatePushPermissionStatus()
  fun diagnose(callback: (Result<List<String>>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: RetenoHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val customDeviceIdValueArg = args[4] as String?
            val isDebugArg = args[5] as Boolean
            val deviceTokenHandlingModeArg = args[6] as NativeDeviceTokenHandlingMode
            val defaultNotificationChannelConfigArg = args[7] as NativeDefaultNotificationChannelConfig?
            val wrapped: List<Any?> = try {
              api.initWith(accessKeyArg, lifecycleTrackingOptionsArg, isPausedInAppMessagesArg, useCustomDeviceIdProviderArg, customDeviceIdValueArg, isDebugArg, deviceTokenHandlingModeArg, defaultNotificationChannelConfigArg)
              listOf(null)
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.reteno_plugin.RetenoHostApi.requestPushPermission$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val provisionalArg = args[0] as Boolean
            api.requestPushPermission(provisionalArg) { result: Result<Boolean> ->
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.reteno_plugin.RetenoHostApi.diagnose$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.diagnose{ result: Result<List<String>> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(RetenoHostApiPigeonUtils.wrapError(error))
              } else {
  ...
  fun onNotificationClicked(payloadArg: Map<String, Any?>, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onNotificationDeleted(payloadArg: Map<String, Any?>, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.reteno_plugin.RetenoFlutterApi.onNotificationDeleted$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(payloadArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
  ...
  fun onCustomNotificationReceived(payloadArg: Map<String, Any?>, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.reteno_plugin.RetenoFlutterApi.onCustomNotificationReceived$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(payloadArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
  ...
  fun onInAppCustomDataReceived(payloadArg: NativeInAppCustomData, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.reteno_plugin.RetenoFlutterApi.onInAppCustomDataReceived$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(payloadArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
